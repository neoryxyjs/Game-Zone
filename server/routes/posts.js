...
router.post('/:postId/comments/:commentId/reply', authMiddleware, async (req, res) => {
  ...
  try {
    // Intentar insertar con las nuevas columnas, si fallan usar las básicas
    let result;
    try {
      ...
      // Si las columnas no existen, usar query básico
      console.log('Columnas reply_to no disponibles, usando query básico');
      result = await pool.query(
        'INSERT INTO comment_replies (comment_id, user_id, content) VALUES ($1, $2, $3) RETURNING *',
        [commentId, user_id, content]
      );
